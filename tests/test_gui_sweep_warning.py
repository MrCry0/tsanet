"""Tests for the sweep value mismatch warning.

The sweep_mismatch_warning function is a pure helper, exercisable
...
def test_no_warning_when_nothing_requested():
    assert sweep_mismatch_warning(None, 100, 200, 450) is None


def test_no_warning_when_points_match():
    assert sweep_mismatch_warning({"points": 450}, 2_400_000_000, 2_490_000_000, 450) is None


def test_warns_when_points_clamped():
    # Real-world case: the device caps points at 450, silently, no error.
    warning = sweep_mismatch_warning({"points": 900}, 2_400_000_000, 2_490_000_000, 450)
...
def test_warns_when_start_frequency_clamped():
    warning = sweep_mismatch_warning({"start": 1}, 100_000, 868_000_000, None)
    assert warning is not None
    assert "start" in warning


def test_warns_when_center_derived_value_differs():
    warning = sweep_mismatch_warning({"center": 433_000_000}, 400_000_000, 500_000_000, None)
    assert warning is not None
...
def test_no_warning_when_points_not_reported_by_device():
    ...
    assert sweep_mismatch_warning({"points": 900}, 100, 200, None) is None


def test_multiple_mismatches_are_all_reported():
    warning = sweep_mismatch_warning(
